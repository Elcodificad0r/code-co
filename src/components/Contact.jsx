...
const TerminosModal = ({ onClose, dark }) => {
  ...
  const headerBorder= dark ? "rgba(255,255,255,0.10)" : "rgba(0,0,0,0.10)";
  ...
  return (
    <div
      ...
      style={{ background: "rgba(0,0,0,0.72)", backdropFilter: "blur(6px)", zIndex: 100002 }}
      onClick={onClose}
    >
      <div
        className="relative w-full max-w-3xl mx-4 my-6 rounded-2xl shadow-2xl flex flex-col"
        style={{ maxHeight: "92vh", zIndex: 100003, overflow: "hidden", backgroundColor: bg }}
        onClick={(e) => e.stopPropagation()}
      >
        <div
          className="sticky top-0 px-8 py-5 flex items-center justify-between"
          style={{ zIndex: 100004, backgroundColor: bg, borderBottom: `1px solid ${headerBorder}` }}
        >
          <button
            onClick={onClose}
        ...
        <div className="font-space" style={{ overflowY: "auto", flex: 1, padding: "2rem", WebkitOverflowScrolling: "touch" }}>
          ...
          {[
            ...
            { num: "12", title: "Modificaciones a estos términos y legislación aplicable", content: `Code&Co. se reserva el derecho de modificar estos términos en cualquier momento. Estos términos se rigen por las leyes vigentes en los Estados Unidos Mexicanos. Cualquier controversia se intentará resolver de forma amistosa; de no lograrse un acuerdo, las partes se someterán a los tribunales competentes de Monterrey, Nuevo León, México.` },
          ].map((section) => (
            <div key={section.num} className="mb-8 pb-8 last:mb-0" style={{ borderBottom: `1px solid ${borderColor}` }}>
              <div className="flex items-baseline gap-4 mb-3">
                <span className="font-rubik80s text-lg leading-none" style={{ color: accentBlue }}>{section.num}</span>
                <h2 className="font-space font-bold text-base tracking-wide" style={{ color: textPrimary }}>{section.title}</h2>
              </div>
              <p className="text-sm leading-relaxed pl-10" style={{ color: textBody }}>{section.content}</p>
            </div>
          ))}
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{ background: "none", border: "none", color: textMuted }}
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="group-hover:-translate-x-1 transition-transform duration-200">
          <path d="M19 12H5M12 5l-7 7 7 7" />
        </svg>
      ...
      {sent ? (
        <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
          <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ background: "rgba(5,32,245,0.1)" }}>
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#0520F5" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            >
              ...
              <option value="Plataforma personalizada">Plataforma personalizada</option>
...
const ContactSection = () => {
  ...
  useEffect(() => {
    if (gsapLoaded && window.gsap) {
      ...
      if (titleRef.current) gsap.fromTo(titleRef.current, { y: 80, opacity: 0, skewY: 2 }, { y: 0, opacity: 1, skewY: 0, duration: 1.2, ease: "power3.out", delay: 0.3 });
      if (widgetRef.current) gsap.fromTo(widgetRef.current, { y: 50, opacity: 0 }, { y: 0, opacity: 1, duration: 1, ease: "power3.out", delay: 0.5 });
      if (asteriskRef.current) gsap.fromTo(asteriskRef.current, { scale: 0.4, opacity: 0, rotation: -45 }, { scale: 1, opacity: 1, rotation: 0, duration: 1.4, ease: "back.out(1.4)", delay: 0.8 });
    }
  }, [gsapLoaded]);
      ...
      if (!existingScript) {
        ...
        script.onload = () => {
          if (window.Calendly && widgetRef.current) {
            window.Calendly.initInlineWidget({ url: "https://calendly.com/holacodenco/30min", parentElement: widgetRef.current });
          }
        };
  ...
  return (
    <>
      {showTerminos && <TerminosModal onClose={() => setShowTerminos(false)} dark={dark} />}
      ...
      <style>{`
        ...
        /* widget-card NO corta el asterisco */
        .widget-card {
          overflow: visible !important;
        }
        @media (max-width: 767px) {
          .asterisk-wrap {
            width: clamp(13rem, 65vw, 22rem) !important;
        ...
        /* ── ELIMINAR ESPACIO BLANCO MOBILE ── */
        @media (max-width: 767px) {
          .contact-outer {
            ...
            <div className="grid grid-cols-1 lg:!grid-cols-2 gap-12 lg:!gap-16 items-center lg:!items-start justify-items-center lg:!justify-items-start text-center lg:!text-left">
              ...
              <div className="space-y-8 w-full max-w-lg lg:!max-w-none">
                <div ref={titleRef} className="space-y-4">
                  <h1 className="font-rubik80s text-5xl md:text-7xl lg:!text-8xl leading-none" style={{ color: textPrimary }}>
                    Hablemos
                    <span className="mt-2" style={{ color: accentBlue }}> de tu</span>
                    <span className="mt-2" style={{ color: textPrimary }}> proyecto*</span>
                  </h1>
                  <p className="font-space text-lg md:text-xl max-w-lg mx-auto lg:!mx-0 lg:!max-w-lg" style={{ color: textMuted }}>
                ...
                <div className="space-y-6">
                  <div className="font-space" style={{ color: textMuted }}>
                    <h3 className="font-bold text-lg mb-3" style={{ color: textPrimary }}>¿Qué incluye la llamada?</h3>
                    <ul className="space-y-2 text-base text-left max-w-md mx-auto lg:!mx-0 lg:!max-w-none">
                      <li className="flex items-start space-x-3">
                        <span className="font-bold" style={{ color: accentBlue }}>01</span>
                        <span>Análisis de tu proyecto actual</span>
                      </li>
                      <li className="flex items-start space-x-3">
                        <span className="font-bold" style={{ color: accentBlue }}>02</span>
                        <span>Propuesta de solución técnica</span>
                      </li>
                      <li className="flex items-start space-x-3">
                        <span className="font-bold" style={{ color: accentBlue }}>03</span>
                  ...
                  <div
                    ...
                    style={{ background: quoteBg, backdropFilter: "blur(8px)", border: `1px solid ${quoteBorder}` }}
                  >
                    <p className="font-space text-sm italic text-left" style={{ color: textMuted }}>
                      "En 30 minutos puedes saber exactamente qué necesitas para llevar tu idea al siguiente nivel"
                    </p>
                    <p className="font-space text-xs mt-2 text-left" style={{ color: textMuted }}>— Code&Co. Team</p>
                  </div>
                </div>
              </div>
